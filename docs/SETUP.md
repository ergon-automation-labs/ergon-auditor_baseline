# Getting Started with bot_army_auditor_baseline

This guide walks you through setting up Baseline Bot for local development.
...
DATABASE_NAME=bot_army_auditor_baseline_dev
```
...
Baseline Bot uses the **BotArmy.GenBot** harness to run skills. Skills are discrete, reusable units of work triggered via NATS messages.

### Creating Your First Skill

1. **Start with the template** — `lib/bot_army_auditor_baseline/skills/example.ex` is a fully documented template
2. **Copy and rename** it for your skill:
   ```bash
   cp lib/bot_army_auditor_baseline/skills/example.ex lib/bot_army_auditor_baseline/skills/my_skill.ex
   ```
...
```elixir
defmodule BotArmyAuditorBaseline.Skills.Classify do
  use BotArmy.Skill

  def name, do: :classify
  def description, do: "Classifies text into categories"
  def nats_triggers, do: ["bot.bot_army_auditor_baseline.command.classify"]
...
```elixir
# In lib/bot_army_auditor_baseline/application.ex or similar

defmodule BotArmyAuditorBaseline.MyBot do
  use BotArmy.GenBot,
    skills: [
      BotArmyAuditorBaseline.Skills.Classify,
      BotArmyAuditorBaseline.Skills.ExtractEntities
    ],
    bot_id: :my_bot
end
```
...
```bash
# Make your changes
edit lib/bot_army_auditor_baseline/skills/my_skill.ex
...
releases: [
  baseline_bot: [
    applications: [bot_army_auditor_baseline: :permanent]
  ]
]
```

This creates a release named `baseline_bot` that is deployed to `/opt/ergon/releases/baseline_bot/` on the server.
...
3. Extracts and deploys to `/opt/ergon/releases/baseline_bot/`
