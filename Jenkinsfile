pipeline {
  ...
  environment {
    BOT_NAME = 'baseline_bot'
    RELEASE_DIR = "/opt/ergon/releases/${BOT_NAME}"
    GITHUB_REPO = "ergon-automation-labs/ergon-auditor_baseline"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/baseline_bot/bin/baseline_bot"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyAuditorBaseline.Release.create()'"
          $RELEASE_BIN eval 'BotArmyAuditorBaseline.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyAuditorBaseline.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyAuditorBaseline.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_auditor_baseline/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_auditor_baseline/releases/start_erl.data)
        fi
